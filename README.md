...
4. Escape the maze by the fastest possible route.
   As the robot explores the maze, it records all the squares it has visited. Once it reaches the target square, one of the outputs is a path variable. The code takes this path and removes all the dead ends from it.
   This is done by pruning the dead ends from the path using a stack. The robot iterates through the path list, adding it to both a stack and a set. if any given coordinates are in the set, this means they are also in the stack.
   Which means that coordinates in between those two are essentially a loop. The robot will remove all coordinates between the first instance and second instance of the same coordinate and repeat this process for the entire path.
...
7. Return to home (20%)
   The beauty of Depth-First Search and backtracking, is that when the robot reaches the final unvisited square, it will backtrack all the way back to the start. This happens on it's own as a core feature of the algorithm. This is essentially the
...
Below I will explain some of the helper functions i've used and why:
